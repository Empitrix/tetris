...
var ASM_CONSTS = {
  103815: () => { if (document.fullscreenElement) return 1; },  
 103861: () => { return document.getElementById('canvas').width; },  
 103913: () => { return parseInt(document.getElementById('canvas').style.width); },  
 103981: () => { document.exitFullscreen(); },  
 104008: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 104081: () => { if (document.fullscreenElement) return 1; },  
 104127: () => { return document.getElementById('canvas').width; },  
 104179: () => { return screen.width; },  
 104204: () => { document.exitFullscreen(); },  
 104231: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 104364: () => { return window.innerWidth; },  
 104390: () => { return window.innerHeight; },  
 104417: () => { if (document.fullscreenElement) return 1; },  
 104463: () => { return document.getElementById('canvas').width; },  
 104515: () => { return parseInt(document.getElementById('canvas').style.width); },  
 104583: () => { if (document.fullscreenElement) return 1; },  
 104629: () => { return document.getElementById('canvas').width; },  
 104681: () => { return screen.width; },  
 104706: () => { return window.innerWidth; },  
 104732: () => { return window.innerHeight; },  
 104759: () => { if (document.fullscreenElement) return 1; },  
 104805: () => { return document.getElementById('canvas').width; },  
 104857: () => { return screen.width; },  
 104882: () => { document.exitFullscreen(); },  
 104909: () => { if (document.fullscreenElement) return 1; },  
 104955: () => { return document.getElementById('canvas').width; },  
 105007: () => { return parseInt(document.getElementById('canvas').style.width); },  
 105075: () => { document.exitFullscreen(); },  
 105102: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 105160: () => { return screen.width; },  
 105185: () => { return screen.height; },  
 105211: () => { return window.screenX; },  
 105238: () => { return window.screenY; },  
 105265: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 105318: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 105389: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 105446: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 105702: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 105773: () => { if (document.fullscreenElement) return 1; },  
 105819: () => { return window.innerWidth; },  
 105845: () => { return window.innerHeight; },  
 105872: () => { if (document.pointerLockElement) return 1; },  
 105919: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 108077: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 108241: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 108345: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 111344: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 111410: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 111803: ($0) => { miniaudio.untrack_device_by_index($0); },  
 111846: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 111971: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
