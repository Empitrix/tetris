...
int move_down(){
	...
	for(int i = 0, cntr = 0; i < MAX_BLKS; i++){ if(buffer[i].movement){ points_y[cntr++] = buffer[i].y; }}
	sort(points_y, N);
	int buffer_limit = (points_y[N - 1] - 1 < AREA_H - 2);
...
int main(void){
	...
	while (!WindowShouldClose()){
		...
		for(int i = 0; i < MAX_BLKS; i++){
			drawRect(V2(buffer[i].x, buffer[i].y),
				(buffer[i].set || buffer[i].movement) ? buffer[i].color : (Color){ 25, 25, 25, 255 },
				BLACK);
		}
